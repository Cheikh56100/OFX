...
def _pdf_has_text(pages_text, min_chars=300):
    ...
    Vérifie : quantité de texte ET densité alphanumérique (évite les PDFs avec
    uniquement des artefacts ou caractères parasites issus de scans).
    """
    total = sum(len(p.strip()) for p in pages_text)
    if total < min_chars:
        return False
    # Vérifier que le texte contient suffisamment de caractères lisibles
    # (not just noise/symbols from failed extraction)
    all_text = ' '.join(pages_text)
    alpha_count = sum(1 for c in all_text if c.isalpha())
    if len(all_text) > 0 and alpha_count / len(all_text) < 0.15:
        return False
    return True
...
def _preprocess_image_for_ocr(img):
    """
    Prétraitement de l'image pour améliorer l'OCR sur relevés bancaires.
    Applique : niveaux de gris, débruitage, binarisation adaptative.
    """
    try:
        import numpy as np
        ...
        # Binarisation : seuil adaptatif via numpy
        arr = np.array(gray)
        # Otsu-like threshold simple
...
def _ocr_pdf(pdf_path):
    """
    OCR robuste : EasyOCR en priorité (meilleure précision sur chiffres),
    fallback Tesseract, avec preprocessing d'image systématique.
    """
    if not _PDF2IMAGE_OK:
        raise RuntimeError("pdf2image non installé — impossible de convertir le PDF en images.")
    if not _OCR_AVAILABLE:
        raise RuntimeError(
            "Ce PDF semble scanné. Installez easyocr ou pytesseract+Tesseract."
        )
    ...
    for img in images:
        ...
        if _EASYOCR_OK:
            try:
                ...
                # Reconstruire le texte en respectant l'ordre vertical (y-coord)
                results_sorted = sorted(results, key=lambda r: (r[0][0][1], r[0][0][0]))
                lines, prev_y = [], -999
                for bbox, text, conf in results_sorted:
                    y = bbox[0][1]
                    if y - prev_y > 15:  # nouvelle ligne
                        lines.append(text)
                    else:
                ...
                if _TESSERACT_OK:
                    pages_text.append(pytesseract.image_to_string(processed, lang='fra+eng'))
                else:
                    pages_text.append('')
        elif _TESSERACT_OK:
            pages_text.append(pytesseract.image_to_string(processed, lang='fra+eng'))
...
def main():
    ...
    for uploaded_file in uploaded_files:
        ...
        # ── Panneau Debug (expander) ──────────────────────────────────────────
        with st.expander("🔧 Informations de diagnostic", expanded=False):
            # Statut OCR
            ocr_status_lines = []
            ocr_status_lines.append(f"**Mode OCR actif :** {'EasyOCR' if _EASYOCR_OK else 'Tesseract' if _TESSERACT_OK else 'Aucun (PDF texte uniquement)'}")
            ocr_status_lines.append(f"**EasyOCR disponible :** {'✅' if _EASYOCR_OK else '❌'}")
            ocr_status_lines.append(f"**Tesseract disponible :** {'✅' if _TESSERACT_OK else '❌'}")
            ocr_status_lines.append(f"**pdfplumber :** {'✅' if _PDFPLUMBER_OK else '❌'}")
            ocr_status_lines.append(f"**pdf2image :** {'✅' if _PDF2IMAGE_OK else '❌'}")
            if ocr_mode:
            ...
            with col_dbg2:
                ...
                if txns_for_ofx:
                    ...
                    # Aperçu 3 premières transactions brutes
