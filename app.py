...
def audit_transactions(txns):
    """Vérifie la qualité des transactions extraites. Retourne une liste d'anomalies."""
    issues = []
    for t in txns:
        if abs(t['amount']) < 0.01:
            issues.append(f"Montant suspect : {t.get('date','')} | {t.get('name','')[:40]}")
        if len(t.get('name', '')) < 3:
            issues.append(f"Label trop court : {t.get('date','')} | '{t.get('name','')}'")
        if not re.match(r'^\d{8}$', t['date']):
            issues.append(f"Date invalide : '{t.get('date','')}' | {t.get('name','')[:30]}")
    return issues


def _generate_excel_export(txns, info, bank_label, currency):
    """Génère un fichier Excel professionnel avec onglets Transactions + Résumé."""
    import openpyxl
    ...
    ws['A1'].font = Font(name='Calibri', size=14, bold=True, color=C_HDR_BG)
    ws['A1'].alignment = Alignment(horizontal='left', vertical='center')
    ws.row_dimensions[1].height = 28

    period = f"{info.get('period_start','')} → {info.get('period_end','')}"
    ws.merge_cells('A2:G2')
    ws['A2'] = f"Période : {period}   |   IBAN : {info.get('iban','')}   |   Devise : {currency}"
    ...
    for ci, (h, w) in enumerate(zip(headers, col_widths), 1):
        ...
        c.font      = Font(name='Calibri', size=10, bold=True, color=C_HDR_FG)
    ...
    cumul = 0.0
    for ri, txn in enumerate(txns, HDR + 1):
        d = txn['date']
        is_debit = txn['type'] == 'DEBIT'
        abs_amt  = abs(txn['amount'])
        cumul   += txn['amount']
        bg = C_DEBIT if is_debit else (C_ALT if ri % 2 == 0 else C_CREDIT)
        row_vals = [
            f"{d[6:8]}/{d[4:6]}/{d[0:4]}",
            'Débit' if is_debit else 'Crédit',
            txn['name'],
            txn.get('memo', '') or '',
            abs_amt if is_debit  else None,
            abs_amt if not is_debit else None,
            cumul,
        ]
        for ci, val in enumerate(row_vals, 1):
            ...
            c.alignment = Alignment(
                horizontal='right' if ci in (5, 6, 7) else ('center' if ci == 2 else 'left'),
                vertical='center'
            )
    ...
    # Ligne TOTAL
    tr = HDR + len(txns) + 1
    td = sum(abs(t['amount']) for t in txns if t['type'] == 'DEBIT')
    tc = sum(t['amount']      for t in txns if t['type'] == 'CREDIT')
    for ci, val in enumerate(['', 'TOTAL', f"{len(txns)} transaction(s)", '', td, tc, cumul], 1):
        c = ws.cell(row=tr, column=ci, value=val)
        c.font   = Font(name='Calibri', size=10, bold=True, color='0F2D6B')
        c.fill   = PatternFill('solid', fgColor=C_TOTAL)
        c.border = bdr
        if ci in (5, 6, 7) and isinstance(val, float):
            c.number_format = fmt_num
    ...
    ws2['A1'].font = Font(name='Calibri', size=13, bold=True, color=C_HDR_BG)
    ...
    for ri, (lbl, val) in enumerate(rows_summary, 2):
        bg2 = 'F7F9FF' if ri % 2 == 0 else 'FFFFFF'
        la = ws2.cell(row=ri, column=1, value=lbl)
        la.font = Font(name='Calibri', size=10, bold=True, color='374151')
        la.fill = PatternFill('solid', fgColor=bg2)
        la.border = bdr
        va = ws2.cell(row=ri, column=2, value=val)
        va.font = Font(name='Calibri', size=10)
        va.fill = PatternFill('solid', fgColor=bg2)
        va.border = bdr
...
def main():
    ...
    for uploaded_file in uploaded_files:
        ...
        ocr_badge = ""
        if info.get('_ocr_mode') == 'claude_vision':
        ...
        elif info.get('_ocr_mode') == 'tesseract':
        ...
        # ── Audit qualité ──────────────────────────────────────────────────────
        audit_issues = audit_transactions(txns_for_ofx)
        if audit_issues:
            st.markdown(
                ...
                f"— vérifiez le tableau ci-dessus.</div>",
                unsafe_allow_html=True
            )
            with st.expander(f"Voir les {len(audit_issues)} anomalie(s)", expanded=False):
                for issue in audit_issues:
                    st.markdown(f"- `{issue}`")

        # ── Téléchargement OFX + Excel ────────────────────────────────────────
        st.markdown('<div class="section-header" style="margin-top:1.2rem">⬇️ Télécharger</div>', unsafe_allow_html=True)
        ...
        with col_dl2:
            if excel_ok:
                st.download_button(
                    ...
                    use_container_width=True,
                    help="Tableau Excel avec onglets Transactions + Résumé, mise en forme automatique",
                )
            else:
                st.markdown(
                    "<div style='font-size:0.8rem;color:#e53e3e;padding-top:0.6rem'>"
                    "❌ Export Excel indisponible — vérifiez que <code>openpyxl</code> est installé.</div>",
                    unsafe_allow_html=True
                )
        with col_dl3:
            st.markdown(f"""
