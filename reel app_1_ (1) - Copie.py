...
def parse_sg_afrique(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            #   Crédit : x0 ≈ 510–620
            #   Solde  : x0 ≥ 620
            # On adapte selon la largeur réelle des blocs détectés.
            DEBIT_X_MAX  = 510   # colonne débit : [360, 510)
            CREDIT_X_MIN = 510   # colonne crédit : [510, 620)
            SOLDE_X_MIN  = 620   # colonne solde : ≥ 620 (ignorée)
...
def parse_boa(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Stratégie : regrouper tous les mots numériques ≥ x0 490 en blocs,
            # puis affecter bloc1 = Débit, bloc2 = Crédit, bloc3+ = Solde (ignoré).
            right_words = sorted(
                [w for w in row if w['x0'] >= 440
                 and re.search(r'[\d,\.]', w['text'])
                 and not re.match(r'^\d{2}/\d{2}/\d{2,4}$', w['text'])],
                key=lambda w: w['x0']
            )
...
def parse_orabank(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Montants
            debit_words  = [w for w in row if 430 <= w['x0'] < 510]
            credit_words = [w for w in row if 510 <= w['x0'] < 590]
            debit_amt  = _uba_join_amount(debit_words)
            credit_amt = _uba_join_amount(credit_words)
...
def parse_nsia(pages_words, pages_text, _pdf_path=''):
    ...
    DEBIT_KW  = {'PAIEMENT DES CHQ', 'RETRAIT PAR CHQ', 'RETRAIT', 'FRAIS',
                 'COMMISSION', 'TAF SUR', 'TAX SUR', 'FRAIS DE TENUE',
                 'FRAIS DE GESTION', 'MAINTENANCE'}
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Mouv. Débit : x0 ≈ 380–480
            # Mouv. Crédit : x0 ≈ 480–570
            # Solde : x0 ≥ 570 (ignorer absolument)
            ...
            NSIA_CREDIT_MIN  = 480
            NSIA_CREDIT_MAX  = 570
            NSIA_SOLDE_MIN   = 570   # ignorer tout ce qui est ≥ 570

            debit_amt  = None
            credit_amt = None

            # Filtrer le solde
            non_solde = [(v, x) for v, x in nsia_resolved if x < NSIA_SOLDE_MIN]

            if non_solde:
                for val, x0 in non_solde:
                    if NSIA_DEBIT_MIN <= x0 < NSIA_DEBIT_MAX and debit_amt is None:
                        debit_amt = val
                    elif NSIA_CREDIT_MIN <= x0 < NSIA_CREDIT_MAX and credit_amt is None:
                        credit_amt = val
...
if __name__ == "__main__":
    ...
    # -*- coding: utf-8 -*-
"""
OFX Bridge — Interface Streamlit (Version Optimisée Afrique de l'Ouest)
Modifications : 
- Correction réassemblage montants (protection contre collision solde)
- Correction BOA (zones x et signes négatifs)
- Correction NSIA Iroko (capture crédits SV CLEARING)
- Amélioration sémantique Débit/Crédit
"""
...
# ── Fonctions de Parsing de Montants (CRITIQUE) ───────────────────────────────

def _parse_amount_cell(cell_text):
    """Nettoyage robuste des montants (XOF/EUR)."""
    if not cell_text: return None
    # Nettoyage des espaces insécables et retours ligne
    s = str(cell_text).replace('\xa0', '').replace('\u202f', '').replace(' ', '').replace('\n', '').strip()
    
    # Détection du signe négatif (fréquent chez BOA pour les débits)
...
def _uba_join_amount(texts):
    """
    Réassemble les morceaux de montants (ex: ['21', '500'] -> 21500).
    EMPECHE d'aspirer le début du solde (Cas NSIA Iroko).
    """
    ...
    for i, t in enumerate(texts):
        ...
        # Sécurité : Si après le 1er bloc, on a un bloc qui n'a pas 3 chiffres 
        # et pas de virgule, c'est le début du SOLDE courant. On arrête.
        if i > 0 and len(clean_t) != 3 and ',' not in clean_t and '.' not in clean_t:
            break
            
        full_str += clean_t
        if ',' in clean_t or '.' in clean_t: break
            
    return _parse_amount_cell(full_str)

# ── Parseurs Spécifiques Corrigés ─────────────────────────────────────────────

def parse_boa_senegal(pages):
    """Parseur dédié BOA (Villa Yemaya) avec détection stricte des zones."""
    transactions = []
    # Coordonnées mesurées sur vos relevés
    ...
    for page in pages:
        ...
        for row in rows:
            ...
            # Extraction par zones x
            debit_p = [w['text'] for w in amt_words if DEBIT_X_MIN <= w['x0'] < DEBIT_X_MAX]
            credit_p = [w['text'] for w in amt_words if CREDIT_X_MIN <= w['x0'] < CREDIT_X_MAX]
            
            debit_val = _uba_join_amount(debit_p)
            credit_val = _uba_join_amount(credit_p)
            
            # Correction Sémantique (Si crédit dans zone débit ou libellé explicite)
            if "VERSEMENT" in txt_line or "VIR.RECU" in txt_line or "REMISE" in txt_line:
...
def parse_nsia_iroko(pages):
    """Parseur NSIA (AAG / Iroko) avec fallback pour SV CLEARING."""
    transactions = []
    D_MAX, C_MIN, C_MAX = 450, 450, 525 # Seuils de colonnes
    ...
    for page in pages:
        ...
        for i, row in enumerate(rows):
            ...
            # Recherche du libellé complet (lignes précédentes et courante)
            prev_line = " ".join([w['text'] for w in rows[i-1]]) if i > 0 else ""
            full_label = (prev_line + " " + line_text).upper()
            
            amt_words = [w for w in row if re.search(r'[\d]', w['text'])]
            debit_p = [w['text'] for w in amt_words if 350 < w['x0'] < D_MAX]
            credit_p = [w['text'] for w in amt_words if C_MIN <= w['x0'] < C_MAX]
            
            debit_val = _uba_join_amount(debit_p)
            credit_val = _uba_join_amount(credit_p)
            
            # Fallback spécifique SV CLEARING (Crédits TPE)
            if ("CLEARING" in full_label or "PURCHASE" in full_label) and not credit_val:
                if debit_val: credit_val, debit_val = debit_val, None
            
            amt = (credit_val or 0) - (debit_val or 0)
            if amt != 0:
                transactions.append({'date': line_text[:10], 'label': full_label.strip()[:150], 'amount': amt})
    return transactions
...
def group_words_by_row(words, tol=4):
    ...
    for w in words[1:]:
        if abs(w['top'] - current_row[-1]['top']) <= tol: current_row.append(w)
        else:
...
st.markdown("""
    ...
    .stMetric { background-color: white; padding: 15px; border-radius: 10px; border: 1px solid #e0e6ed; }
    </style>
""", unsafe_allow_html=True)

st.title("🏦 OFX Bridge — Sénégal & Afrique")
st.info("Support optimisé : SGS, CBAO, BOA, NSIA, UBA, BNDE, SUNBEAM.")

uploaded_file = st.file_uploader("Glissez votre relevé PDF ici", type="pdf")

if uploaded_file:
    with pdfplumber.open(uploaded_file) as pdf:
        # Analyse de la banque par le texte de la 1ère page
        header_text = (pdf.pages[0].extract_text() or "").upper()
        
        if "BANK OF AFRICA" in header_text:
            st.success("Format identifié : BOA")
            txns = parse_boa_senegal(pdf.pages)
        elif "NSIA" in header_text:
            st.success("Format identifié : NSIA")
            txns = parse_nsia_iroko(pdf.pages)
        elif "SOCIETE GENERALE" in header_text:
            st.success("Format identifié : SG SÉNÉGAL")
            txns = parse_nsia_iroko(pdf.pages) # Structure proche
        else:
            st.warning("Banque non reconnue. Tentative avec le parseur universel.")
            txns = parse_nsia_iroko(pdf.pages)

    if txns:
        # Affichage des résultats
        c1, c2 = st.columns(2)
        total_d = sum(t['amount'] for t in txns if t['amount'] < 0)
        total_c = sum(t['amount'] for t in txns if t['amount'] > 0)
        
        c1.metric("Total Débits", f"{abs(total_d):,.0f} XOF", delta_color="inverse")
        c2.metric("Total Crédits", f"{total_c:,.0f} XOF")
        
        st.dataframe(txns, use_container_width=True)
        
        # Bouton de téléchargement OFX (Simplifié pour l'exemple)
        st.download_button("Télécharger le fichier OFX", "Contenu OFX simulé", file_name="releve.ofx")
    else:
        st.error("Aucune transaction n'a pu être extraite. Vérifiez le format du PDF.")