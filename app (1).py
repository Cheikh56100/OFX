...
def parse_boa(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Libellé : x0 ≈ 100–340 (exclut Date val à ~68 et Référence à ~346)
            label_words = [w for w in row if 98 <= w['x0'] < 342]
            label = ' '.join(w['text'] for w in label_words).strip()
            label_up = label.upper()
            ...
            while j < len(rows):
                ...
                if r2 and any(w['x0'] < 65 and re.match(r'^\d{2}/\d{2}/\d{2,4}$', w['text'])
                               for w in r2):
                    break
                nl = ' '.join(w['text'] for w in r2 if 98 <= w['x0'] < 342).strip()
...
def process_pdf(file_bytes: bytes, filename: str, force_ocr: bool = False, _version: int = 12):
    """
...
def main():
    ...
    for uploaded_file in uploaded_files:
        ...
        with st.spinner(f"Analyse de « {uploaded_file.name} »…"):
            bank, info, txns, error = process_pdf(file_bytes, uploaded_file.name, force_ocr=force_ocr, _version=12)
        ...
        if not txns:
            ...
            if info.get('_empty_period') or (info.get('iban') and info.get('period_start')):
                ...
                # Vérifier si le PDF contient réellement du texte extractible
                import pdfplumber as _plumber
                _has_text = False
                try:
                    with _plumber.open(tmp_path) as _pdf_check:
                        _total = sum(len((p.extract_text() or '').strip()) for p in _pdf_check.pages)
                        _has_text = _total >= 100
                except Exception:
                    pass

                if not _has_text and not _OCR_AVAILABLE:
                    # Vrai PDF scanné sans OCR disponible
                    st.warning(
                    ...
                    # PDF natif avec texte, mais parseur n'a rien extrait
                    st.warning(
                        f"⚠️ **Aucune transaction détectée** pour ce relevé **{bank}**. "
                        "Le PDF contient du texte mais la mise en page n'a pas pu être interprétée. "
                        "Vérifiez que le fichier est bien un relevé bancaire valide, "
                        "ou utilisez l'éditeur pour corriger manuellement."
                    )
                st.markdown('</div>', unsafe_allow_html=True)
                continue
